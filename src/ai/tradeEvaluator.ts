...
export function evaluateTradeForBot(
  ...
  const gainedProperties = isRecipient
    ? trade.offeredProperties
    : trade.requestedProperties;
  const lostProperties = isRecipient
    ? trade.requestedProperties
    : trade.offeredProperties;

  const gainedJailCards = isRecipient
    ? (trade.offeredJailCards.chance ? 1 : 0) +
      (trade.offeredJailCards.communityChest ? 1 : 0)
    : (trade.requestedJailCards.chance ? 1 : 0) +
      (trade.requestedJailCards.communityChest ? 1 : 0);
