...
export function useBotRunner(state: GameState, dispatch: React.Dispatch<GameAction>) {
  ...
  useEffect(() => {
    ...
    if (state.turnPhase === 'GAME_OVER' || !state.isAutoPlaying) {
      return;
    }
      ...
      if (bidder && bidder.isAI && !bidder.isBankrupt) {
        timeoutRef.current = setTimeout(() => {
          ...
          if (typeof decision === 'number') {
            dispatch({ type: 'PLACE_AUCTION_BID', payload: { playerId: currentBidderId, amount: decision } });
          } else if (decision === 'PASS') {
            dispatch({ type: 'PASS_AUCTION_BID', payload: { playerId: currentBidderId } });
          } else {
            dispatch({ type: 'EXIT_AUCTION', payload: { playerId: currentBidderId } });
          }
        }, delay);
      }
      return;
    ...
    if (state.turnPhase === 'TRADE' && state.activeTrade) {
      ...
      if (recipient && recipient.isAI) {
        timeoutRef.current = setTimeout(() => {
          const accepted = evaluateTradeForBot(state, recipient.id, state.activeTrade!);
          if (accepted) {
      ...
      timeoutRef.current = setTimeout(() => {
        if (currentPlayer.inJail) {
          ...
          if (jailDecision === 'USE_CARD') {
            const cardType = currentPlayer.getOutOfJailCards.chance > 0 ? 'chance' : 'communityChest';
            dispatch({ type: 'USE_JAIL_CARD', payload: { cardType, playerId: botId } });
            return;
    ...
    if (state.turnPhase === 'LANDED_ACTION') {
      timeoutRef.current = setTimeout(() => {
        ...
        if (wantsBuy) {
          dispatch({ type: 'BUY_PROPERTY', payload: { propertyIndex: currentPlayer.position } });
        } else {
          dispatch({ type: 'DECLINE_BUY', payload: { propertyIndex: currentPlayer.position } });
        }
      }, delay);
      return;
    }
    ...
    if (state.turnPhase === 'DEBT_RESOLUTION' && state.debtInfo) {
      timeoutRef.current = setTimeout(() => {
        const plan = evaluateBotDebtLiquidation(state, botId, state.debtInfo!.amountOwed);
        if (plan.canSurvive && plan.actions.length > 0) {
          ...
          dispatch({ type: 'DECLARE_BANKRUPTCY', payload: { playerId: botId } });
        }
      }, delay);
      return;
    }
