...
describe('Turn Progression and Orchestration', () => {
  ...
  it('updates bot speed settings', () => {
    ...
    state = gameReducer(state, { type: 'SET_BOT_SPEED', payload: { speed: 'fast' } });
    expect(state.botSpeed).toBe('fast');

    state = gameReducer(state, { type: 'SET_BOT_SPEED', payload: { speed: 'instant' } });
    expect(state.botSpeed).toBe('instant');
  });

  it('declares winner and GAME_OVER when only 1 active player remains', () => {
    let state = createInitialGameState();
    // Bankrupt players 1 through 7
    for (let i = 1; i <= 6; i++) {
    ...
    state = gameReducer(state, { type: 'DECLARE_BANKRUPTCY', payload: { playerId: 7 } });
