...
describe('Monopoly Game Engine & Rules Reducer', () => {
  ...
  it('moves player and awards $200 when passing GO', () => {
    ...
    state = gameReducer(state, { type: 'ROLL_DICE', payload: { diceOverride: [2, 3] } });
  ...
  it('sends player to jail on 3 consecutive doubles', () => {
    ...
    state = gameReducer(state, { type: 'ROLL_DICE', payload: { diceOverride: [4, 4] } });
  ...
  it('allows player to buy unowned property', () => {
    ...
    state = gameReducer(state, { type: 'BUY_PROPERTY', payload: { propertyIndex: 39 } });
  ...
  it('handles landing on Go To Jail (square 30)', () => {
    ...
    state = gameReducer(state, { type: 'ROLL_DICE', payload: { diceOverride: [2, 2] } }); // land on 30
  ...
  it('handles income tax and luxury tax', () => {
    ...
    state = gameReducer(state, { type: 'ROLL_DICE', payload: { diceOverride: [1, 3] } });
  ...
  it('handles mortgaging and unmortgaging with 10% fee', () => {
    ...
    state = gameReducer(state, { type: 'MORTGAGE_PROPERTY', payload: { propertyIndex: 39 } });

    expect(state.properties[39].isMortgaged).toBe(true);
    expect(state.players[0].money).toBe(1700); // 1500 + 200

    expect(canUnmortgageProperty(state, 0, 39)).toBe(true);
    state = gameReducer(state, { type: 'UNMORTGAGE_PROPERTY', payload: { propertyIndex: 39 } });
