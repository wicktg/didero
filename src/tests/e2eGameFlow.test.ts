...
describe("End-to-End 8-Player Game Simulation", () => {
  it("runs a continuous 100-turn multi-player match deterministically without runtime crashes", () => {
    ...
    for (let turn = 0; turn < 120; turn++) {
      ...
      if (state.turnPhase === "ROLL") {
        if (activePlayer.inJail) {
          ...
          if (
            jailDecision === "USE_CARD" &&
            (activePlayer.getOutOfJailCards.chance > 0 ||
              activePlayer.getOutOfJailCards.communityChest > 0)
          ) {
            const cardType =
              activePlayer.getOutOfJailCards.chance > 0
                ? "chance"
            ...
            state = gameReducer(state, {
              type: "USE_JAIL_CARD",
              payload: { cardType, playerId: activePlayer.id },
            });
      ...
      if (state.turnPhase === "LANDED_ACTION") {
        ...
        if (wantsBuy) {
          state = gameReducer(state, {
            type: "BUY_PROPERTY",
            payload: { propertyIndex: activePlayer.position },
          });
        } else {
          state = gameReducer(state, {
            type: "DECLINE_BUY",
      ...
      while (state.turnPhase === "AUCTION" && state.activeAuction) {
        ...
        if (typeof bidDecision === "number") {
          state = gameReducer(state, {
            ...
            payload: { playerId: bidderId, amount: bidDecision },
          });
    ...
    const totalPropertiesOwned = Object.values(state.properties).filter(
      (p) => p.ownerId !== null,
    ).length;
