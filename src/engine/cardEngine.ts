...
export function drawCard(state: GameState, deckType: 'chance' | 'communityChest'): { state: GameState; card: Card } {
  let deck = deckType === 'chance' ? [...state.chanceDeck] : [...state.communityChestDeck];
  let discard = deckType === 'chance' ? [...state.chanceDiscard] : [...state.communityChestDiscard];
  const fullDeck = deckType === 'chance' ? CHANCE_CARDS : COMMUNITY_CHEST_CARDS;

  if (deck.length === 0) {
    deck = shuffleArray(discard.length > 0 ? discard : fullDeck.map((_, i) => i));
    discard = [];
  }
  ...
  const nextState: GameState = {
    ...
    communityChestDeck: deckType === 'communityChest' ? deck : state.communityChestDeck,
    communityChestDiscard: deckType === 'communityChest' ? discard : state.communityChestDiscard,
    lastDrawnCard: card,
  };
...
export function applyCardEffect(state: GameState, card: Card, playerId: number): GameState {
  let nextState = { ...state };
  ...
  nextState.gameLog = [
    createLogEntry(`${player.name} drew ${deckName}: "${card.text}"`, 'card', playerId),
    ...nextState.gameLog,
  ];
  ...
  switch (card.action.type) {
    ...
    case 'GENERAL_REPAIRS': {
      ...
      nextState.gameLog = [
        createLogEntry(`${player.name} paid $${totalCost} for property repairs.`, 'card', playerId),
        ...nextState.gameLog,
      ];
      if (player.money < 0) {
        nextState.turnPhase = 'DEBT_RESOLUTION';
    ...
    case 'GO_TO_JAIL': {
      ...
      nextState.gameLog = [
        createLogEntry(`${player.name} was sent directly to Jail!`, 'jail', playerId),
        ...nextState.gameLog,
      ];
      break;
    }
  ...
  if (nextState.turnPhase !== 'DEBT_RESOLUTION' && nextState.turnPhase !== 'LANDED_ACTION') {
    const isDoubles = nextState.dice[0] === nextState.dice[1] && !player.inJail && nextState.consecutiveDoubles > 0;
...
export function handleLanding(
  ...
  switch (square.type) {
    ...
    case 'GO_TO_JAIL': {
      ...
      nextState.turnPhase = 'END_TURN';
      nextState.gameLog = [
        createLogEntry(`${player.name} landed on Go To Jail and was arrested!`, 'jail', playerId),
        ...nextState.gameLog,
      ];
      break;
    }
    ...
    case 'TAX': {
      ...
      nextState.gameLog = [
        createLogEntry(`${player.name} paid $${taxAmount} for ${square.name}.`, 'rent', playerId),
        ...nextState.gameLog,
      ];
      if (player.money < 0) {
        nextState.turnPhase = 'DEBT_RESOLUTION';
    ...
    case 'UTILITY': {
      if (prop.ownerId === null) {
        ...
        if (rent > 0) {
          ...
          owner.money += rent;
          nextState.gameLog = [
            createLogEntry(`${player.name} paid $${rent} rent to ${owner.name} for ${square.name}.`, 'rent', playerId),
            ...nextState.gameLog,
          ];
