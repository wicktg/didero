...
describe("Groq API Client", () => {
  it("routes correct API key per agent ID", () => {
    expect(getApiKeyForAgent(0)).toBe(
      "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",
    );
    expect(getApiKeyForAgent(1)).toBe(
...
describe("Agent Action Validator", () => {
  it("validates legal actions and falls back gracefully on illegal actions", () => {
    ...
    const validRoll = validateAndSanitizeAgentAction(state, 0, {
      type: "ROLL_DICE",
    });
    expect(validRoll.type).toBe("ROLL_DICE");

    // Illegal action during ROLL phase
    const fallback = validateAndSanitizeAgentAction(state, 0, {
      type: "BUY_PROPERTY",
